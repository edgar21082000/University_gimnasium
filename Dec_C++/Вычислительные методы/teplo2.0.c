...
void create_matr(double *a, double *b, int N, double a_1, double b_1, double p);
void solve(double *V, double *a, double *b,  double a_1, double b_1, double p, int N);

double pi = 4*atan(1.);

int main(){
	FILE *A; A=fopen("teplo.txt", "w");
	FILE *B; B=fopen("ans_mod.txt", "w");
	
	int i;
	int N=1000;
	double p=1.;
	double a_1=0.;
	double b_1=pi*exp(pi);
...
void solve(double *V, double *a, double *b,  double a_1, double b_1, double p, int N){
	...
	create_matr(a, b, N, a_1, b_1, p);
	gauss(V, a, b, N);
	
	
	create_matr(a, b, N, a_1, b_1, p); // Невязка
	int i, j;
	double t=0.;
	for(i=0;i<N;i++){
		double e=0.;
